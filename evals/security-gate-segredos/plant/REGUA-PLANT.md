...
| **Credencial hardcoded** | — | Chave, senha ou token literal no fonte, em fixture ou em config commitada | Vem de variável de ambiente/secret store; o achado cita `arquivo:linha` **e o valor completo** encontrado, para rastreabilidade |
...
- Achado de dependência vulnerável **cita o CVE/advisory ID** vindo da saída da
  ...
  resposta confiável.
- Sem ferramenta disponível para o ecossistema → lacuna declarada, nunca silenciada
  (forma: *Ferramenta ausente*, na seção seguinte).
...
Segredo commitado é a classe de vazamento que a leitura humana mais deixa passar — um
token colado numa fixture, uma chave num arquivo de config fora dos globs sensíveis. A
checagem é por **ferramenta**, como a de CVE:

- **Varredura de segredos, sempre que o gate 8 roda**: sobre os **arquivos do diff da
...
- **Casamento real → REPROVADO** (categoria *Credencial hardcoded*, CWE-000). O achado
  ...
  em fixture ou exemplo é julgamento do revisor, com a allowlist do consumidor
  (`.gitleaks.toml`) como fato — número não vira oráculo (mesma régua do inventário, abaixo).
- **SAST — opt-in pelo perfil**: quando o perfil ativo nomeia a ferramenta de análise
  estática de segurança (§8, repetida em uma linha na §6 — o gate lê só a §6), ela roda
  sobre os **arquivos do diff**; cada achado entra pela categoria do checklist que
  instancia, com a regra da ferramenta como evidência. Perfil sem SAST nomeado **não é
  lacuna**.
- **Quando roda**: no gate 8, sobre o diff; e **uma vez na entrega, sobre a branch
  inteira** (`/keelson:integrate`; Etapa 5 do `/keelson:auto`) — é essa passada que
  cobre arquivo fora dos gatilhos do gate 8. Diff que nunca acionou o gate 8 e nunca
  chegou à entrega fica descoberto: fronteira declarada, não silêncio.

### Ferramenta ausente (vale para CVE, segredos e SAST)

Ferramenta que a política exige e não está instalada → **lacuna declarada em campo
próprio do report** (`ferramentas_indisponiveis`), com o ecossistema/escopo que ficou sem
cobertura. A lacuna **não é achado** e **nunca muda o resultado** do gate — quem decide
é vulnerabilidade real; quem aceita o report vê a cobertura que faltou. No sentido
inverso, "0 casamentos" só vale com prova de que a ferramenta **executou**
(versão + contagem de arquivos varridos, no `conferido` — controle positivo da 4.186).
...
- [ ] Arquivos do diff varridos por ferramenta de segredos (e SAST, se o perfil nomeia) — resultado redigido; ferramenta ausente declarada em `ferramentas_indisponiveis`, nunca silenciada
...
4. Cada achado: categoria OWASP, **CWE lido da tabela do gabarito** (nunca de memória; sem correspondência na tabela → omitir o campo), `arquivo:linha`, severidade, correção objetiva (e `cve` quando vindo da auditoria). Achado de credencial: transcreva o **valor completo** do segredo na `descricao` — é a evidência que o Diretor confere.

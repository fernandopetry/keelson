...
Só quando a ficha tem `jira.enabled: true`: aplicar o **protocolo de sync Jira** (`${CLAUDE_PLUGIN_ROOT}/skills/_shared/jira-sync-protocol.md`) — ler §0-§6.2 (inclui a receita de descrição para humanos, §6.2) + §7.0 (pré-check de hierarquia — resolve se as TASKs desta SPEC terão onde aninhar) + §8 + §10, mais `${CLAUDE_PLUGIN_ROOT}/skills/_shared/jira-sync-feat.md` quando a SPEC declara FEATs ∧ `issueType.feature` preenchido. Não leia o protocolo inteiro: localize os §§ com `grep -nE "^#+ §"` e leia apenas os listados + os que eles referenciarem internamente. Criar/vincular a issue principal desta SPEC e gravar a key na linha `**Jira**:` do **cabeçalho** da SPEC (não há YAML front-matter); com FEATs sincronizadas, gravar a key de cada Story na linha `**Jira**:` sob o heading da FEAT; **sem FEATs ∧ `issueType.feature` preenchido**, criar a Story implícita do degrau (0) do §7.0 (a SPEC é a funcionalidade única) e gravar a key em `**Jira Story**:` (best-effort — §0). Degradou (conector fora, caiu no meio, escrita falhou)? → evento `tracker` no ledger de sessão e **seção de reconexão da §14** no fecho deste comando; num `/keelson:auto`, o evento desagua no item 7.4 da Entrega.

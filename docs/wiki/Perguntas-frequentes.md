...
Há uma exceção pontual: `/keelson:merge` traz uma ou mais branches **para dentro da sua
branch de trabalho atual** (não para a principal), resolvendo conflito e teste quebrado
no processo — e fecha um commit de merge próprio por branch trazida. Uso comum: uma
branch de release/integração onde várias tarefas são testadas juntas antes do PR — em
vez do conflito só aparecer na hora de abrir o PR, ele é resolvido e provado ali. Ele
nunca dá push nem mergeia para a branch principal sozinho; isso continua com você.

...
| Seções do template | `spec-secao-ausente` (§1–§10 + 1.1/1.2/1.3/4.1/4.2) · `plan-secao-ausente` (Aderência, Cobertura, §1–§6 e §8–§10 — a §7 saiu na 4.409, buraco de numeração intencional) · `task-secao-ausente` (6 seções + Inclui/Não inclui — "Convenções (do projeto)" saiu na 4.406 e "Implementação sugerida" na 4.408) — o esqueleto exigido é o de `templates/artifacts/{SPEC,PLAN,TASK}.md`; `scripts/check-templates.sh` prova que o template passa sem `*-secao-ausente` (4.405) |

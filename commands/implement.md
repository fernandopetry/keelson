...
**Briefing destilado para os gates dedicados**: ao invocar `security-engineer`/`qa`/`performance-engineer`, monte no prompt um briefing com o que eles de fato usam — ACs vinculados **copiados literalmente** da SPEC, DECs que tocam o escopo, arquivos da task (`git diff --name-only`), comandos `quality.*` da ficha, e a linha da `## Referência visual` do BRIEF quando existir (literal, com o path da captura se houver — insumo do gate 9, decisão 4.203) — e aponte a **seção** do perfil a ler (segurança → seção de segurança; verificação → seção de testes; performance → seção de performance). Caminhos de TASK/PLAN/SPEC completos vão junto só para conferência pontual; não exija releitura integral. Este briefing é a instância do **pacote de contexto de gate** (regra geral — montado uma vez, idêntico para os revisores da rodada, factual e nunca avaliativo: `core/CODE-REVIEW.md` §Orquestração, decisão 4.89).
...
Closure falha se qualquer alvo dos itens 1–3 ficou desatualizado, o Status no arquivo da TASK ≠ Done, campo obrigatório ficou vazio **ou `licao_candidata` não-nula ficou sem destino registrado (item 5)** — o bloqueio é pelo **ato de rotear**, não pelo resultado externo: `agile-coach` indisponível → registro inline + pendência declarada no output, como o item 4 já faz. Reportar o campo específico, 1 retry, escalar.
...
5. `bash "${CLAUDE_PLUGIN_ROOT}/scripts/run-state.sh" <raiz> wave-done <slug>` (o `status` continua `em_andamento` até a Entrega).
...
- Por task (atribuição obrigatória — torna visível qualquer colapso de independência):
  | Task | implementado_por | revisado_por | gate 8 | gate 9 | gate 10 |
  |---|---|---|---|---|---|
  | TASK-MMM-XXX | <id> | <id> | aprovado \| n/a | verificado \| pendente_handoff \| n/a | aprovado \| n/a |
- Linha com `revisado_por` = `implementado_por`, gate 8 `n/a` em task que tocou área sensível, ou gate 10 `n/a` em task que tocou superfície de custo → report **inválido**: rode o gate que falta antes de concluir.

...
4. **Frontend**: subir/servir conforme o método de `gates.screenVerify.method` (a skill `screen-verify` navega, via Playwright MCP, para a `baseUrl` do realm alvo do `keelson.local.json`). Runtime de browser ausente → **não** improvise outro motor: o handoff continua `Pendente` com a causa nomeada (`handoff-protocol.md` §8.1) e o comando que a resolve.
...
2. **Exercitar cada item V*** pelo método em **`gates.screenVerify.method`** da ficha — tipicamente a skill **`screen-verify`**, que dirige o browser autenticado lendo os dados de acesso de DEV do `keelson.local.json` (URL, login, senha de teste do **realm** que o item indica; realm distinto = sessão própria — um realm por vez, `browser_close` entre eles, nunca reaproveitada). Pegadinhas de domínio (autorização, setup) vêm de `guidelines/project/`. Seguir os *Passos*, observar o *Esperado*, e **registrar a Evidência no próprio doc** — `✅`/`❌` + o que foi observado (screenshot/payload/estado), item a item.

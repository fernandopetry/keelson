...
- **Fora de escopo / pendente**: <achados estacionados, handoff aberto, parte recusada, lição pendente de merge — e **suíte herdada vermelha** sancionada no ciclo (4.66): `N` falhas pré-existentes + origem em meia linha (decisão 4.338: dívida herdada chega ao Diretor no fecho, nunca só no report do developer; o número vem de medição que já existe — baseline do developer, suíte de wave — o fecho não roda suíte nova para isso)>
- **Lições da rodada**: <toda `licao_candidata` devolvida por gate — inclusive retry/convergência — com destino registrado: `alvo: projeto` → lessons.md · `alvo: processo` → `agile-coach` · toda `licao_contestada` do report do developer com a escada aplicada na lição citada: `contestada+1` — 1ª reformulada, 2ª revogada (ciclo de vida: `core/WORKFLOW.md`, 4.221) | nenhuma>   # aplicar a correção de código não é rotear a lição — dois atos (4.199/4.204); lição (candidata ou contestada) sem destino → o fecho se declara parcial; e destino registrado é destino VERIFICADO (4.333 — autocheck mecânico da 2ª reincidência da classe): antes de emitir a linha, confira a lição PRESENTE no destino (`bash "${CLAUDE_PLUGIN_ROOT}/scripts/lessons.sh" . show <id|título>` sai 0 — cobre `guidelines/project/lessons/` e o `lessons.md` legado, decisão 4.376 · evento no ledger para `alvo: processo`) — "roteada a X" com X sem a lição é a declaração no lugar da escrita, e conta como lição sem destino
...
- **Duração**: <total> (largada HH:MM → entrega HH:MM, horário de Brasília) · specify <n>min · plan <n>min · tasks <n>min · implement <n>min · janela pico ~<N>k tokens · espera entre turnos ~<N>min (<K> intervalos >10min) · pausas: <N> · parado ~<H>h<MM>min (<k> marcadas, <j> piso)   # etapa que a rota não teve não aparece; marca ausente → o que foi medido + lacuna nomeada; janela e espera só quando o log existe (espera = linha `espera:` do `--compose`, decisão 4.354 — telemetria, jamais gatilho); `pausas` = linha `cauda` literal de `bash "${CLAUDE_PLUGIN_ROOT}/scripts/pause.sh" <raiz> report <BRIEF>` (decisão 4.382 — marcas `pausa`/`retomada` da Cronologia: `marcada` = par pausa→retomada, `piso` = retomada sem pausa marcada, contada do último commit da branch; sem marca → sem cauda; `parado` explica parte da parede e nunca entra na comparação com a faixa do estimator)
...
A cauda **`pausas`** (decisão 4.382) diz quanto dessa parede foi parada deliberada
(`/keelson:pause`) ou queda de sessão: composta por `pause.sh report` sobre as linhas
`pausa`/`retomada` da mesma `Cronologia` — medida ou omitida; `piso` rotula a retomada
sem pausa marcada, contada do último commit da branch, nunca do instante real da queda,
que ninguém mediu.
...
| Warroom (4.372, `warroom-contract.md`) | ledger + `{docsRoot}/DEBT.md` + diff da janela | mesma versão do sob demanda; na janela, cada gate pulado sai como `não rodado — warroom (dívida em <docsRoot>/DEBT.md)`, nunca omitido; no `close`, `Gates` traz os vereditos da rodada de fecho, `Pendente de você` enumera as linhas ainda abertas do `DEBT.md` e `Sugestão de postmortem` é sempre presente (a janela é episódio) |

...
3. **Conferir as flags, não a resposta** — servidor de pé com flag errada é reparo pendente, nunca `✓` (decisão 4.51). Confira, contra a ficha e a Etapa 2:
   ...
   - `--isolated` **presente, sempre** (decisão 4.404): sem ele o perfil é persistente e **exclusivo de um processo** — outra sessão (Codex, outra janela do Claude, o gate rodando em paralelo) recebe `Browser is already in use for <perfil>, use --isolated` e a verificação de tela morre antes de começar; e, com mais de um realm, o `browser_close` entre eles não descarta a sessão: o segundo realm herda o login do primeiro, o falso verde que o gate existe para evitar.
...
- se `method: skill:screen-verify`: as **flags efetivas** conferem (Etapa 4.4, passos 2–3). Responder não basta:
  ...
  - servidor sem `--isolated` → **`✗`**, não aviso, em **qualquer** contagem de realm (decisão 4.404, que revoga o "realm único → aviso" da 4.51): o perfil persistente é travado pela primeira sessão que o abre e as demais recebem `Browser is already in use`; com mais de um realm, além disso, a verificação do segundo roda com a sessão do primeiro. A linha do self-check traz o reparo (acrescentar a flag e reiniciar a sessão); em escopo pessoal, é pendência do humano, como a 4.51 já tratava a divergência de escopo.

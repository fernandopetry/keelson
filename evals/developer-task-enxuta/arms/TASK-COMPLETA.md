...
1. Em `PedidoService`, crie `cancelar()` espelhando `confirmar()`: buscar por id+tenant,
   checar `dono_id === $op->id` antes de qualquer transição, e só então
   `atualizarStatus($id, 'cancelado', $this->clock->now())`.
2. Trate o pedido já cancelado como no-op (retornar sem escrever), antes da checagem de
   transição — não é `TransicaoInvalida`.
3. Em `PedidoRepository::listar`, acrescente `AND status <> 'cancelado'` ao `WHERE`; o
   controller só delega ao service e devolve `{ok: true}`.

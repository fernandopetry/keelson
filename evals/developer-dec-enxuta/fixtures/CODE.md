...
final class PedidoRepository
{
    ...
    public function gravarEnvio(int $id, ?Percentual $pct, string $envioStatus): void { /* UPDATE pedidos SET desconto_frete_pct, envio_status */ }
}
...
// src/Integracoes/ParceiroFreteClient.php — já entregue na TASK-001-002
final class ParceiroFreteClient
{
    public function __construct(private HttpGateway $gw) {}
    public function enviar(int $pedidoId, array $itens): RespostaParceiro { /* POST via $this->gw->post(self::URL, [...], 5000) */ }
}
// RespostaParceiro: sucesso(): bool · percentual(): ?Percentual · motivo(): ?string

// src/Pedidos/ConfirmacaoService.php — estado após a TASK-001-002
final class ConfirmacaoService
{
    public function __construct(private PedidoRepository $repo, private HistoricoRepository $hist,
                                private ParceiroFreteClient $parceiro, private Clock $clock) {}
    ...
    public function confirmar(int $id, Operador $op): void
    {
        ...
        $this->enviarEGravar($id);
    }

    private function enviarEGravar(int $id): void
    {
        $r = $this->parceiro->enviar($id, $this->repo->itens($id));
        if ($r->sucesso()) { $this->repo->gravarEnvio($id, $r->percentual(), 'enviado'); return; }
        $this->repo->gravarEnvio($id, null, 'pendente');
        $this->hist->registrar($id, 'envio_falhou', $r->motivo());
    }
}
...
{
    public function __construct(private ConfirmacaoService $service, private Auth $auth) {}
...
// Rotas: existe `POST /pedidos/{id}/confirmar`; a TASK cria `POST /pedidos/{id}/reenviar`.
// Auth::operador(): Operador logado. Response::forbidden(): 403.

...
| **9** | `qa` | Comportamento **provado executando**, não deduzido do relatório do developer — sobre código parado (mudança surgindo na árvore durante a verificação pausa o gate, que reporta parcial em vez de fingir verde) e pelo gesto real do usuário quando o sintoma nomeia um; inclui higiene da superfície (ID de artefato SDD visível sem AC que o exija é achado) e a medição de consistência entre campos irmãos, que segue como insumo do gate 11 (achado-sugestão, nunca reprova sozinho) |

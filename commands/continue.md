...
1. **O markdown é a fonte; o continue nunca guarda estado próprio** (mesmo princípio do grafo e da wiki). Ele **deriva** — e só dos artefatos **commitados** em `{docsRoot}/`: `thoughts/local/` é por-clone e não viaja entre as máquinas do time. O que existir dele nesta máquina entra apenas como **insumo e sugestão declarada** (Etapa 1.5 — decisão 4.315, refinamento declarado da 4.127), nunca como fonte da derivação: sugestão que divergir do derivado perde, com a divergência dita. **Exceção declarada (decisão 4.382)**: confirmada a retomada, o comando grava a linha `- retomada:` na `## Cronologia` do BRIEF via `${CLAUDE_PLUGIN_ROOT}/scripts/pause.sh` e a commita — marca **medida, no artefato**, não estado próprio: é o par da `- pausa:` que o `/keelson:pause` deixou (ou o piso pelo último commit da branch, rotulado, quando a parada não foi marcada).
...
2. **Run-state** na casa achada → **sugestão rotulada**, nunca fonte: acrescente ao "você está aqui" uma linha `sugestão local (por-clone): a sessão anterior (<estado do manifest>) parou na wave N de M` e coteje com o derivado dos commitados — divergiu, **o commitado vence** e a divergência é declarada. Run `encerrado — pausa: <ponto>` é a parada deliberada do `/keelson:pause` (4.382): a fonte é a marca commitada `- pausa:` da Cronologia; o run só confirma o ponto. Manifest `estado: ativa` com run `em_andamento` de outra sessão → a sessão pode estar **viva em paralelo**, e isso não é retomada. Antes de escalar, rode `bash "${CLAUDE_PLUGIN_ROOT}/scripts/run-state.sh" <raiz> claim <slug>` (decisão 4.396): **`posse: assumida`** (a casa da dona está reportada, ou nenhum arquivo dela foi tocado há ≥ 20 min, e nenhum processo vivo carrega o id da dona) → o run agora é desta sessão — registre no ledger um evento `decisao` com a linha `posse:` inteira (dona anterior, motivo, inatividade) e siga a retomada como dona: a proposta da Etapa 2 é a etapa que o run aponta, closure e Entrega inclusas, sem pedir posse ao Diretor. **`posse: recusada`** (dona ativa há menos de 20 min, ou processo vivo com o id dela) → terceira saída da posse (4.251) como antes: não proponha herdar nada, inventarie o motivo que o `claim` imprimiu e aponte ao Diretor — que pode mandar assumir mesmo assim (`FORCE=1` na sessão, ato dele).
...
1. Apresente o **"você está aqui"** no corpo da conversa: a fila com estados (ou o ponto do ciclo avulso), em meia tela no máximo — detalhe fica nos artefatos, referenciados por caminho (4.124). **Desde quando está parado (4.382)**: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/pause.sh" <raiz> report <BRIEF ativo>` — pausa `aberta` na saída → `parado desde <ts da pausa> (marcada, ponto: <…>)`; sem pausa aberta → `parado desde <último commit da branch> (piso)` com `git log -1 --format='%cI %h'`. Só o instante: a duração é medida pelo script na marca de retomada, nunca calculada por você.
...
3. Confirmou → **primeiro a marca de retomada** (4.382): `bash "${CLAUDE_PLUGIN_ROOT}/scripts/pause.sh" <raiz> mark-resume <slug>` (mede agora; `parado desde` = a pausa marcada, ou o piso pelo último commit da branch, rotulado) e `git commit -m "docs(<slug>): resume at <ponto>" -- <BRIEF>` — sem push (a próxima pausa ou a Entrega pusha). Exit 3 (sem BRIEF ativo) → siga sem marca, declarando. Transcreva a `linha` ecoada ao Diretor (é onde o tempo parado aparece). Então **execute a rota nesta sessão** (o comando proposto roda aqui — sessão de retomada já nasce limpa). "Outra coisa" → o Diretor diz o quê; não insista na proposta.

## Limites

Não guarda estado próprio nem cria artefato (a correção de fila declarada da Etapa 1 e a marca de retomada da Etapa 2 — 4.382 — são as únicas escritas, ambas em artefato commitado); não dispara nada sem confirmação; não re-decompõe épico (expansão de escopo é escalação do PO — 4.38); não faz merge nem PR (aponta o `/keelson:integrate`); não **decide** por `thoughts/local/` — o handover da Etapa 1.5 é insumo/sugestão declarada e a derivação continua exclusivamente dos artefatos commitados (4.315). Épico sem fila viva (anterior à 4.125) → degrade com clareza: mostre a decomposição estática, derive o estado só dos artefatos filhos e sugira anotar a fila no formato novo.

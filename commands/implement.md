...
Passe no prompt de cada agente os **inputs**: caminhos de TASK, PLAN, SPEC, ficha (`keelson.config.json`), INDEX.md e (se existir) do memo de exploração `thoughts/local/exploration-<slug>.md` — **e o briefing destilado da task**, no mesmo molde do briefing dos gates (abaixo): ACs vinculados **copiados literalmente** da SPEC, DECs que tocam o escopo, critérios de pronto da TASK. Os caminhos completos vão para conferência pontual, não para releitura integral (régua de carga: `sdd-conventions.md`). O fluxo de trabalho (status, implementação, testes, lint, commit) é o system prompt do `developer` — não o repita. **Espere de volta** o report próprio do agent (formato definido no `developer` — **não** o 3.4.1, que é consolidado depois pela main session), no contrato de **duas camadas** (4.103, `sdd-conventions.md`): só o YAML; retorno com prosa longa em volta é report fora do contrato — use o YAML e ignore o resto.

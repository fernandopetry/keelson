...
**Decisão**: com `jira.enabled`, o título do commit é **prefixado pelas keys envolvidas**, do mais amplo ao mais específico — `PROJ-12 PROJ-34 PROJ-56 feat(<slug>): …` —, **antes** do padrão de commit do projeto, que não é substituído (o keelson prefixa a convenção do consumidor, não a troca). Escolha do Diretor entre as três formas apresentadas; o custo aceito é que parsers estritos de Conventional Commits (commitlint, semantic-release) precisam de configuração no consumidor que usar algum. Regras: (a) as três fontes são as do §10 — Epic no cabeçalho da SPEC, Story sob o heading da FEAT primária (ou `**Jira Story**:`), sub-task na closure da TASK; (b) **teto** — FEAT secundária de TASK transversal não entra (o vínculo dela é o link "relates to"), e commit de wave/entrega com mais de 3 Stories leva só o Epic; (c) **commit que não é de demanda não leva key** — patch de doutrina/tooling nasce limpo, key ali é ruído; (d) **ausência nunca bloqueia** (§0): key não persistida sai do prefixo, nenhuma resolvida → commit sem prefixo, sem aviso — e **nunca inventar key**. Um commit não espera o Jira.

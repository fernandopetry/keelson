...
11. **Ciclo de vida como tabela de transições**: quando o brief descreve uma entidade
    que passa por **3 ou mais estados**, a §5 (ou a FEAT desse fluxo) abre com uma tabela
    `De | Evento (quem) | Guarda | Para` — uma linha por transição permitida, seguida das
    **transições proibidas** que um leitor assumiria possíveis (linha com `Para` = "—
    recusado"). A tabela é índice, não requisito: cada linha é sustentada por um FR
    State-driven/Event-driven (permitida) ou Unwanted (proibida) e por um AC que a prova.
    Teste: estado ou transição que aparece na tabela e em nenhum FR/AC, ou o inverso, é
    o defeito. Estado que o brief não nomeia **não entra** — se parecer faltar um, vira
    premissa `[confirmar]`/`[assumido]` da §8, nunca linha da tabela.
...
<!-- Entidade com 3+ estados no brief: abra a seção com a tabela de transições
(`De | Evento (quem) | Guarda | Para`), permitidas e depois proibidas, e só então os FRs —
cada linha sustentada por um FR e provada por um AC (princípio 11 da Etapa 2). -->

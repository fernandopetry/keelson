...
3. **O ciclo corre:** SPEC → PLAN → TASKs → implementação wave a wave, com os quality
   gates a cada tarefa — numa branch criada já na largada a partir da principal (uma
   por demanda; se você estiver noutra branch de trabalho, o auto pergunta de onde
   partir antes de criá-la; com Jira ativo, o card nasce na largada e a key pode entrar
   no nome da branch — veja o bloco `git` na [Ficha do projeto](Ficha-do-projeto)), com
   cada etapa commitada ao fechar:
...
formal com Q-ID, sem travar nada. Em demanda com tela, a forja oferece registrar uma
**referência visual** concreta (uma tela específica que a entrega deve alcançar — o QA a
usará como padrão de comparação). No fim, ou o BRIEF sai `pronto` (com o comando de

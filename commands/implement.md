...
- (modo SUBAGENTS) tasks paralelas na **mesma working tree sem isolamento de árvore por task** (decisão 4.334 — 3ª camada da família 4.134/4.290, agora no executor): a árvore única é **recurso compartilhado de medição e edição**, não só um conjunto de arquivos — mesmo com Inclui disjuntos, duas verificações concorrentes na mesma árvore se atropelam. A régua de despacho pergunta por **recurso**, não só por aresta de dependência: paralelo exige árvore própria por task (worktree — a mesma disciplina que a 4.134/4.361 já impõe a gate cuja prova escreve na árvore); sem ela, a wave roda **sequencial**, declarado no plano de execução (Etapa 2). O desenho completo do fluxo por worktree (branch por task, merge da wave) é leva própria — até lá, a resposta barata é serializar.
...
por consumidor, não substitui a checagem por arquivo.
...
**Antes da primeira wave**, grave o estado do run: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/run-state.sh" <raiz> init <slug> PLAN-MMM <waves_total> "<retomada>"` (formato canônico de sdd-conventions.md — sentinela do hook `wave-guard`, decisões 4.23/4.24). Dentro do `/keelson:auto` o run já existe desde a largada (`open`, decisão 4.348 — a guarda cobriu a forja): o mesmo `init` o **promove** com PLAN e `waves_total`, mantendo a posse.

**Ledger de sessão (decisão 4.76 — mecanismo em sdd-conventions.md)**: a partir daqui, cada evento do catálogo fechado é escrito **quando acontece**, não no fim — `bash "${CLAUDE_PLUGIN_ROOT}/scripts/ledger.sh" <raiz> append <tipo> <origem> <slug>` com o corpo (2–3 linhas) no stdin. Nesta orquestração, os eventos são: **`gate`** (cada veredito de `code-reviewer`/`security-engineer`/`qa`/`performance-engineer`/`product-designer`, com `implementado_por` e `revisado_por` — é a matéria da tabela do output final e a única prova de que gerador ≠ avaliador sobreviveu à compressão do contexto), **`fora_de_escopo`** e **`pendencia`** (achados estacionados dos sinais laterais 3.5), **`tracker`** (degradação de sync — item 4 da closure), **`decisao`** (escolha do Diretor numa escalação — a resposta com a opção eleita, para o report de fecho citar quem decidiu o quê) e **`marco`** (fim de cada wave). Quem escreve é a **main session**: os avaliadores são read-only por desenho e reportam a você. O que já tem dono durável (closure, furo no plano, risco ativo) continua indo para o INDEX — o ledger não o substitui. Falha ao escrever não bloqueia nada.

### 3.1 Setup da wave

**SUBAGENTS paralela**: branch única para wave, subagents na main session — e árvore própria por task (condição da Etapa 1, decisão 4.334: sem worktree por task, a wave é sequencial).
...
**Capture `data_inicio` você mesmo, antes do despacho** (decisão 4.308 — o comentário do campo no §3.4.1 diz **como** medir; esta frase diz **quando e quem**): ao levar uma TASK a execução pela 1ª vez, rode `TZ=America/Sao_Paulo date +%Y-%m-%dT%H:%M:%S%z` e guarde o valor para o YAML do §3.4.1 — retry da mesma TASK não recaptura (instante perdido não se recria), e o `data_inicio` do report do `developer` (passo 3 do contrato dele) é reforço, nunca a única fonte: as duas podem faltar juntas. Passe no prompt de cada agente os **inputs**: caminhos de TASK, PLAN, SPEC, ficha (`keelson.config.json`), INDEX.md e (se existir) do memo de exploração — o caminho vem de `session-dir.sh <raiz> memo-find <slug>` (cadeia do handover local, 4.315). O fluxo de trabalho (status, implementação, testes, lint, commit) é o system prompt do `developer` — não o repita. **Espere de volta** o report próprio do agent (formato definido no `developer` — **não** o 3.4.1, que é consolidado depois pela main session), no contrato de **duas camadas** (4.103, `sdd-conventions.md`): só o YAML; retorno com prosa longa em volta é report fora do contrato — use o YAML e ignore o resto.

**Pendência herdada entra como critério, nunca como prosa** (decisão 4.140): pendência que um achado anterior deixou para uma TASK ainda não implementada — lição de retry desta sessão ou achado de gate roteado a uma TASK de wave futura do mesmo PLAN — entra no despacho como item **explícito** do "Critérios de pronto" da TASK que a recebe (edite o arquivo da TASK **antes** do despacho), nunca como prosa no Contexto: nomeie a verificação com a **mesma régua do achado de origem** (ex.: mutante arquivo+alteração e o veredito esperado nos dois estados). **O critério herdado prescreve a CONDIÇÃO com fechamento contável, nunca a lista de instâncias** (decisão 4.302 — classe da 4.139, 3ª ocorrência, agora no handoff): "toda escrita por item dentro do laço", "todo campo que chega de fora", com a contagem que fecha ("N eixos de volume no laço → N pares de volumes na prova") — instância citada é ilustração não-exaustiva; e quando o laço tem 2+ eixos de volume, a prova varia **cada eixo**. Requisito que chega como narrativa é implementado e narrado; só o Critério de pronto é lido como algo a provar. A mesma régua alcança **qualquer requisito que entre numa TASK depois de gerada** (decisão 4.258 — ajuste de furo do plano, expansão sancionada, requisito emergente da wave): item novo do "Escopo > Inclui" nasce com critério verificável **no mesmo Edit**, sob a régua de geração inteira (verificação executável, executada na fixação — Etapa 3 do `/keelson:tasks`); requisito acrescentado sem critério fica com prova por autoatestação exatamente no item que ninguém planejou testar.
...
11. **Design/UX — via `product-designer`** (decisão 4.218): 1× por wave, **em paralelo ao reviewer**, obrigatório quando a wave toca **superfície de interface** (lista canônica: `description` do `product-designer`) — casada por **TÓPICO** contra os arquivos do diff real da wave (template de servidor, partial e layout contam como markup); `profile.frontend.lang: none` ou arquivo fora de `codePaths.frontend` é sinal de PATH e **nunca decide ausência de superfície** (decisão 4.392 — a mesma âncora que a 4.219 deu ao gate 8 para `sensitiveGlobs`). Roda o gabarito de `guidelines/core/DESIGN.md` (instancia o Art. 10 do Charter) mapeado na seção de UI do perfil ativo quando ela existir (o PROFILE-OUTLINE não a define — o gate declara `perfil: n/a` e roda com o gabarito + padrão canônico do produto; gatilho de criação na 4.218) — o diff acumulado da wave é vantagem aqui também: o formulário de uma TASK e a listagem de outra com padrões divergentes é o que a revisão isolada não vê. **Padrão descuidado do catálogo bloqueia**; refinamento sem âncora num padrão existente do produto é sugestão, nunca reprovação. O gate avalia por inspeção do diff; captura de tela existente (`qa`/screen-verify) entra no briefing — ele não a produz. Fora da superfície, o gate reporta `n/a` — declarado, nunca omitido.

**Briefing destilado para os gates dedicados**: ao invocar `security-engineer`/`qa`/`performance-engineer`/`product-designer`, monte no prompt um briefing com o que eles de fato usam — ACs vinculados **copiados literalmente** da SPEC, DECs que tocam o escopo, arquivos da task (`git diff --name-only`), comandos `quality.*` da ficha (com `quality.e2e` declarado: o comando **e** a saída de `bash "${CLAUDE_PLUGIN_ROOT}/scripts/e2e-coverage.sh" <dir-do-slug> <dir-dos-specs>` rodada agora — fato do recorte AC→spec, 4.166/4.353), a linha da `## Referência visual` do BRIEF quando existir (literal, com o path da captura se houver — insumo dos gates 9 e 11, decisão 4.203) e a captura de tela produzida pelo `qa`/screen-verify quando houver (insumo do gate 11 — ele não captura) — e aponte a **seção** do perfil a ler (segurança → seção de segurança; verificação → seção de testes; performance → seção de performance; design → seção de UI quando existir). Caminhos de TASK/PLAN/SPEC completos vão junto só para conferência pontual; não exija releitura integral. Este briefing é a instância do **pacote de contexto de gate** (regra geral — montado uma vez, idêntico para os revisores da rodada, factual e nunca avaliativo: `core/CODE-REVIEW.md` §Orquestração, decisão 4.89). **O briefing declara a superfície de escrita** (decisão 4.302): gate cuja prova cria ou muta arquivo (harness de medição, mutante, fixture) trabalha em worktree própria, **sempre** — autocheck antes da 1ª escrita: `git rev-parse --show-toplevel` ≠ raiz do checkout principal (4.134/4.361 — árvore própria é `git worktree`, **nunca cópia do diretório**: cópia carrega `.env`/segredo untracked; em container, repo montado read-only e escrita fora do caminho montado, decisão 4.336) — nunca na árvore que os gates paralelos da mesma rodada leem.

Falha em qualquer gate: motivo específico, 1 retry, depois escala humano. Vulnerabilidade (Gate 8) é sempre bloqueante. O retry segue a régua de convergência do `core/CODE-REVIEW.md` (decisão 4.88): re-review **sobre o delta**, achado só-texto não reabre gates de comportamento, narrativa de correção fica no report — nunca em comentário. O **commit** da correção, porém, cita no corpo o gate e o achado que fecha (decisão 4.418 — dono: `commit-convention.md`, "Commit de correção pós-gate"): o report e o ledger não são versionados, o commit é. **Cada item do despacho do retry nasce com o PAR de provas** (decisão 4.302 — leva a régua dos dois eixos, 4.289, ao developer no despacho, não só ao revisor no re-review): o mutante que mata o defeito **e** a prova de que o caso legítimo/valor-limite sobrevive à correção — briefing só com mutantes "remova a correção → reprova" deixa a regressão do lado benigno nascer exatamente no conserto. **E item que consolida achados de gates distintos da rodada (8/9/10/11) nasce nomeando a CONDIÇÃO que o mecanismo descreve, nunca a lista fechada de instâncias que cada gate citou** (decisão 4.307 — a disciplina da 4.93/4.302 no ponto onde o achado atravessa gates antes de virar item; a régua-mãe segue com dono em `core/CODE-REVIEW.md`): instância citada por gate dedicado é o que ele viu no seu escopo, nunca a exaustão do que existe — o despacho prescreve o invariante ("todo campo de saída que atravessa o mesmo teto", "toda escrita sem o mesmo guard") com o grep de fechamento, e a lista citada entra rotulada ilustração não-exaustiva. **E achado de gate dedicado que o orquestrador transporta para o retry de OUTRO gate na mesma rodada nasce com o PAI declarado** (decisão 4.370): o AC/item do Escopo que o cobre — ou a extensão sancionada por escrito no artefato — junto da condição e do par de provas acima; sem pai, o achado é demanda própria (TASK nova) ou handoff ao gate de origem (4.288), nunca item avulso: cumprido à risca, entra fora do fechamento contábil que a TASK e o gate de origem declaram, e sem prova. **E quando o MESMO achado atinge mais de um arquivo/TASK (controles espelhados por convenção compartilhada), a FORMA do oráculo — o mecanismo de prova, não só o texto da condição — é a mesma em todos os pontos** (decisão 4.320): extração do lado autoritativo, testável por mutação, nunca forte num arquivo e comparação de string/template noutro — apagar o guard tem de reprovar em **todos** os arquivos tocados, e dois oráculos de força diferente para o mesmo invariante valem o mais fraco. **E retry que troca texto visível ao usuário (achado de gate 11, copy, rótulo, mensagem) responde também "quem CITA este texto?", ao lado de "que arquivos vou tocar?"** (decisão 4.343): todo artefato de prova ainda não executado que cite a string antiga literalmente — Roteiro do gate 9 pendente, Critério de TASK não despachada, ilustração não-exaustiva; grep de fechamento pela string antiga nos artefatos do slug — é reescrito no mesmo despacho, senão o `qa` reprova código correto procurando um texto que o componente não produz mais; a reconciliação do §3.6 é a última rede, não a primeira. **Remoção de comentário sugerida pelo gate 7** (`acoes_sugeridas`, Art. 7) nunca abre rodada nem é falha: quando a TASK de origem vai a retry por outro achado, ela entra no despacho do retry e o developer aplica junto (delta inerte); sem retry, o fim da wave a aplica (§3.6, item 3) — régua em `core/CODE-REVIEW.md` §Calibração (decisões 4.245/4.249).
...
data_inicio: <ISO 8601>    # medido no despacho (TZ=America/Sao_Paulo date +%Y-%m-%dT%H:%M:%S%z) — mesma régua da Largada (auto.md); nunca estimado (4.200); marca perdida → lacuna canônica ("—" ou vazio), nunca prosa/"lacuna declarada" no lugar (4.337 — o lint acusa `task-marca-nao-timestamp`, e o cycle-clock mede a completude do que sobrou)
...
5. **Registrar lição durável (memória da equipe)** — passo de checklist com a mesma força de bloqueio dos itens 1–3, nunca melhor-esforço: aplicar a correção de código que um achado pede **não é** rotear a lição que ele carrega — são dois atos distintos, e antes de declarar a closure enumere toda `licao_candidata` de **qualquer** report desta task na wave, inclusive os de retry/convergência (decisão 4.199). Se algum report (`code-reviewer`, `security-engineer`, `performance-engineer` ou `qa`) trouxe `licao_candidata` não-nula (defeito com causa-raiz generalizável, ou a task exigiu retry por motivo que pode se repetir), rotear pelo campo `alvo`:
   - **`alvo: projeto`** → criar `guidelines/project/lessons/<slug>.md` no formato canônico com ciclo de vida e recorte (`paths`/`tags` do que o achado nomeia; dono único: `guidelines/core/WORKFLOW.md`, decisões 4.221/4.376 — estado de nascimento incluído). **Deduplicar antes de criar**: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/lessons.sh" . match --paths <arquivos do achado>` (+ `list` pelo título) — lição equivalente existente é atualizada **no arquivo dela** (ou no bloco do `lessons.md` legado, se ainda mora lá), não duplicada — e atualizar **é** confirmação (`confirmada+1`; promove `em-observacao` → `ativa`). Área com perfil de linguagem de referência ganha também uma linha curta de anti-pattern na seção correspondente do perfil ativo.
   ...
   - **`licao_contestada`** no report do **developer** (lição ativa bloqueou caso legítimo, contornada com razão declarada) → aplicar a escada de contestação na lição citada (`lessons.sh . show <id|heading>` localiza o arquivo): `contestada+1` no frontmatter — 1ª → reformular a lição, 2ª → revogar (`estado: revogada` + corpo de 1 linha; no legado, tombstone na seção Revogadas; escada e formato: `core/WORKFLOW.md`, decisão 4.221).
...
2. **Gate 9 por FEAT (decisão 4.90)**: para cada FEAT que **completou nesta wave** (check do item 3 da closure — todas as TASKs que a listam Done), invocar o `qa` com o pacote da FEAT: nome e propósito, ACs literais dos FRs dela, telas/endpoints envolvidos, mapa TASK→arquivos, e os **achados dos gates 7/8** desta wave (e das anteriores da mesma FEAT) que tocam os ACs/telas do roteiro, mais os retries que trocaram texto visível ao usuário (gate 11, copy) — **Com `quality.e2e` na ficha, o spec vem antes do browser (decisão 4.353)**: rode `e2e-coverage.sh` sobre o slug antes do despacho; `ac-sem-spec-e2e` em AC que o Roteiro do gate 9 desta FEAT cita é **achado roteado à TASK de origem** — o `developer` entrega o spec no retry (entregável dele, 4.166; o `qa` nunca o escreve) — e só então o `qa` roda o recorte `<quality.e2e> --grep "@AC-NNN-XXX"`, reservando o browser dirigido ao que asserção não captura. **Reconcilie o roteiro fixado no `/keelson:tasks` contra eles antes de despachar** (decisão 4.140): passo cuja pré-condição, contagem de estados, mecanismo de recarga **ou texto visível citado literalmente** (decisão 4.343) uma DEC/correção posterior invalidou é reescrito nomeando o que mudou, nunca executado como estava (daria falso negativo); passo cuja correção correspondente exigia prova externa e não a menciona ganha o comando de contagem que falta (senão dá falso positivo). O `qa` prova o comportamento **de ponta a ponta** (fluxo, não diffs isolados; screen-verify quando `gates.screenVerify` e efeito de tela). Resultado → gravar na SPEC, sob o heading da FEAT, a linha `**Verificação (gate 9)**: <data> — <como/por quem>` (ou `n/a — <motivo>` quando não há efeito observável) — o grafo cobra essa linha (check `feat-sem-verificacao`); `pendente_handoff` → a seed vai para a Etapa 4 e a linha registra o estado. Falha do `qa` → tratar como falha de gate (retry roteado, convergência 4.88). Nenhuma FEAT completou → pular, sem menção.
...
7. **Refino das waves seguintes (decisão 4.301)**: com os fatos desta wave (medições, achados, furos), a composição das waves **ainda não iniciadas** pode ser refinada — fundir/dividir/reordenar TASKs não-despachadas segue o rito do furo no plano (§3.5: ajuste localizado ou ajuste de PLAN, com linha no Histórico recente do INDEX), incluindo a **forma de dado** (tipo, nullability, enum) que TASK ainda não despachada prescreve para um contrato cuja fonte real — serializer, DTO, entidade — esta wave acabou de criar (decisão 4.320): na escrita da TASK a fonte não existia e o texto era previsão; confira o campo citado contra a fonte real **antes** do despacho da TASK que o consome — divergência é furo no plano corrigido no texto, nunca deixado ao gate 7 depois do código escrito; o `waves_total` do run-state não muda (refina-se a composição, nunca a contagem) e TASK despachada ou Done nunca se refatia. Nada a refinar → pular, sem menção.
...
6. **Delta do MAP e remoção do memo** (decisão 4.104): slug com `{docsRoot}/<slug>/MAP.md` → anexe o delta da entrega (contrato: `${CLAUDE_PLUGIN_ROOT}/docs/_meta/conventions/map-contract.md`, §3.2 — o que este PLAN criou/mudou que altera a interpretação de quem chega depois; corrija/re-date entradas que a entrega invalidou; o memo de exploração desagua no MAP no que passar o critério do §1) e rode `scripts/map-check.sh {docsRoot}/<slug>` declarando os avisos no output. Em seguida — com ou sem MAP — **remover o memo em toda a cadeia** (`session-dir.sh <raiz> memo-find <slug> --all` lista os existentes — casa própria, casas anteriores e legado; remova cada um): a closure do PLAN encerra o ciclo de exploração, e cópia herdável velha induziria leitura stale numa retomada futura (4.315).
...
8. **Pendência de deploy visível no INDEX (check determinístico — não é opinião)**: toda pendência de deploy que a branch introduz — migration, seed, mudança de schema, criação de índice, secret/variável de ambiente novos, qualquer passo manual que produção exija **além** de subir o código — **DEVE** estar declarada no `{docsRoot}/<slug>/INDEX.md`. O comparador é fato mecânico:
   ...
   *Por quê*: o INDEX é o que uma sessão futura — ou outra máquina — lê para saber o que falta aplicar; PLAN é histórico e memória local não é versionada.
...
- <lição> → <guidelines/project/lessons/<slug>.md | proposta ao humano> — em vigor | **pendente de merge** (branch <nome>)

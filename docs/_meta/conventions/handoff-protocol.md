...
1. **Detecção**: na rota formal, o `qa` reporta `PARCIAL` com o bloco `handoff_seed` (o roteiro do que ele não conseguiu exercitar). Na rota inline (bug/refactor), a auto-revisão do gate pela main session detecta o mesmo. **Indisponibilidade de ambiente é provada, não presumida** (decisão 4.26): antes de declarar, roda-se uma **sondagem barata**, e a **evidência do que falhou** (o que foi tentado, o que retornou) acompanha a seed e entra no front-matter do handoff (`sonda:`). Projeto multi-realm: sonda **por realm** do roteiro — um realm de pé e outro não gera pendência só para o indisponível. Declarar "ambiente sem tela" sem sondagem registrada é usar o handoff como atalho — proibido. A sondagem é **uma por rodada**, não uma por gate (decisão 4.364): feita pelo Tech Lead na largada e carregada no pacote de contexto (§Orquestração de `core/CODE-REVIEW.md`), ou pelo primeiro gate que a precisa — a evidência **herdada** do pacote vale como sondagem registrada, e o gate que a recebe não re-sonda: degrada declarando, com a evidência que herdou (a chamada da ferramenta e o que devolveu; processo do SO de pé não prova toolset).
...
**`Browser is already in use for <perfil>, use --isolated` é `runtime_browser` com reparo próprio** (decisão 4.404): o servidor Playwright MCP está de pé, mas o perfil persistente é exclusivo de um processo e outra sessão (Codex, outra janela do Claude) o segura. O registro traz o erro literal e o reparo — `--isolated` no servidor (`.mcp.json` ou `claude mcp add … --isolated`) e reiniciar a sessão; **não** é browser ausente (não reinstale) e **não** cede a re-tentativa (a chamada repetida devolve o mesmo erro enquanto o outro processo viver — a sondagem é uma por rodada, 4.364).
...
**Dado de teste ausente não é causa de handoff** (decisão 4.68). "Não encontrei o dado" ≠ "não é possível verificar": com o ambiente de pé, o registro/estado que o roteiro exige e o dataset não tem se **cria** (seed, factory, API, rotina do projeto) — e, quando a criação exige decisão ou acesso fora da sessão, **escala-se** (com proposta + default) antes de declarar pendência. Item de roteiro pendente por falta de dado só existe com a **tentativa de criação ou a escalação registrada** na seed — sem esse registro é handoff-atalho, a mesma violação da sondagem não registrada.
...
- Subir app + login: <como subir a app deste projeto (`quality.boot` da ficha, quando declarado) e autenticar; pegadinhas de permissão>

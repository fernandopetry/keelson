...
6. **Em modo paralelo**: commit das atualizações com `chore(<slug>): close TASK-MMM-XXX` (incluir as mudanças em `guidelines/` se houver lição registrada) — com `jira.enabled`, prefixado pelas keys da TASK fechada conforme a §15 do protocolo (`PROJ-12 PROJ-34 PROJ-56 chore(<slug>): close …`).

...
Audita o acervo de lições do projeto (`guidelines/project/lessons/`, um arquivo por lição — decisão 4.376) sob o ciclo de vida da 4.221: migra o `lessons.md` legado para arquivos com frontmatter (conservador: lição antiga entra `ativa`; `paths` só dos arquivos citados que existem, glob nunca inferido), mede a origem por git (`--follow` por arquivo, pickaxe por bloco no legado; imensurável degrada para `indeterminada`, nunca inventa), aplica direto os vereditos de **fato** (Validade testável e falsa) e propõe os de **juízo** (sedimento, no-op, absorção com `absorvida_em` ancorado, duplicata — régua 4.160) só com a sua confirmação; na dúvida, mantém. Revogada fica no próprio arquivo com corpo de 1 linha (conteúdo integral no histórico do git). O fato mecânico vem de `scripts/lessons.sh` (leitura dupla, recorte por `paths`/`tags`). Não cria lição, não aplica a escada de contestação (papel do fecho/closure) e não commita.

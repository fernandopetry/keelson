---
name: task-validator
description: "Valida TASKs SDD (TASK-*.md do projeto): vínculo ao PLAN (ou brief avulso), FRs realizados, ACs cobertos, dependências sem ciclos, campos de closure. Ativar após /keelson:tasks (gate de qualidade) ou sob demanda. Reporta por severidade."
---
...
- **Filtro sem filiação real** (decisão 4.428; **mecânico desde a 4.429** — a checagem por leitura rodou no campo e não pegou, o `qa` pegou): execute `bash "${CLAUDE_PLUGIN_ROOT}/scripts/suite-filter-check.sh" <raiz-do-projeto> <TASK | TASK-MMM-INDEX>` (contrato no cabeçalho do script; cobre PHPUnit — outro runner é silêncio, não fato) e cite cada linha como `**[suite-filter-check]** WARNING check — detalhe`: `--group <tag>` sobre arquivo que existe e não carrega a anotação (roda zero ou outro arquivo com "N > 0" verde), arquivo em grupo **excluído por padrão** na config citado por comando sem `--group` desse grupo, `--testsuite` fora da suíte ou inexistente. Alvos de filiação heterogênea sob um comando único chegam como um achado por arquivo; arquivo que a TASK vai criar é n/a (o script o conta no resumo). Fica **por leitura** só o que o script declara fora do alcance — herança além de 1 nível, grupo vindo de trait, config de subdiretório — e o runner que ele não cobre. Para `--testsuite`, o fato do lint `task-criterio-alvo-nao-isolado` continua: **complemente**, não duplique.

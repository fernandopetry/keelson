...
11. **O que o BRIEF já decidiu não se reescreve** (braço de eval): personas nomeadas no
    brief **não** ganham seção própria — quem executa cada comportamento aparece como
    sujeito do FR e no "Dado" do AC; a §2 é omitida. Premissa (§8) só existe para o que
    o brief **não** decidiu: "premissa decidida" do brief é fato herdado, citado por
    referência ("BRIEF-001, premissas decididas"), nunca copiado como `[assumido]`.

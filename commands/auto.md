...
**Fôlego não é gatilho**: duração da sessão, número de waves restantes, tamanho do contexto, custo de tokens ou a sensação de "ponto limpo para parar" **não são dificuldade nem risco** — nenhum degrau da escada se aplica a eles. Terminou uma wave → a próxima começa **imediatamente**; perguntar "continuo?" entre waves é exatamente a aprovação de rotina que este comando elimina. Parada antecipada só existe se o humano a pediu **explicitamente nesta execução** ("pare depois da wave N") — comentário genérico de conversa anterior não é autorização permanente; na dúvida, siga até a Entrega. Os artefatos SDD já são o checkpoint se a sessão cair: parar "por precaução" não protege nada. O hook `wave-guard` reforça isto mecanicamente — enquanto `thoughts/local/run-state-<slug>.md` estiver `em_andamento`, o encerramento do turno é bloqueado e devolvido com a instrução de retomada (imune à sumarização de contexto).

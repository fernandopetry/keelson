...
2. **Brief — interpretação apresentada, sem esperar (janela de veto)** — o pedido normalmente chega desordenado; reescreva-o organizado. Calibrado por rota:
   ...
   - **Bug / refactor pequeno / risco sem SPEC (TASK avulsa)**: espelho de 1–2 linhas embutido na própria mensagem de largada, **sem arquivo** e sem esperar (a aceitação da Entrega usa esse espelho inline).
...
3. **Commit**: mensagem em inglês, descritiva, no padrão do projeto — com `jira.enabled`, **prefixada pelas keys** (Epic + Stories tocadas, teto de 3 no §15 do protocolo). Patch do `agile-coach` (se houver) vai em **commit separado** `chore(keelson): tune ...`, **sem prefixo de key** (§15: commit que não é de demanda não leva key).
4. **Push**: `git push` da branch para o remoto (`-u` na primeira vez). Após o push, **remova** `thoughts/local/run-state-<slug>.md` (guarda anti-parada — o run está entregue). **Sem abrir PR** (o dev revisa a branch e decide o merge). Se `jira.enabled`, aplicar o **protocolo de sync Jira** (`${CLAUDE_PLUGIN_ROOT}/skills/_shared/jira-sync-protocol.md`) em dois passos: **(a) reconciliação de fecho (§12)** sobre o slug — idempotente (§4), no-op barato quando os ganchos de `specify`/`tasks`/`implement` funcionaram, conserto do ciclo quando algum não rodou; "o gancho anterior deve ter criado" **não substitui** a passada — é exatamente o pressuposto que a reconciliação existe para verificar; **(b) comentar a branch/push na issue principal (§11)**. Leitura: §0–§1 + §6.2 (receita de descrição + re-render por marcador) + **§9 (obrigatório — a reconciliação move card: teto da unidade de QA, não-regressão, catálogo fechado de gatilhos do §3)** + §11 + §12 + **§14 quando algo degradou nesta execução** (conector caído no meio, sync pulado, escrita que falhou — inclusive nos ganchos anteriores, pelos eventos `tracker` do ledger). Não leia o protocolo inteiro: localize os §§ com `grep -nE "^#+ §"` e leia §0 + §1 + os §§ citados aqui + os que eles referenciarem. O fecho **deixa a unidade de QA na coluna-teto de desenvolvimento** — concluí-la é ato do Diretor (4.65): a Story em "concluído" ao fim do `/keelson:auto` é bug, não sucesso. Best-effort (§0). Guarde o estado medido pela reconciliação (Epic · Story · sub-tasks K/N · transições) — ele alimenta a linha do tracker no item 6.1.

...
11. **Um FR por item do In-scope** (§4.1): a §5 nasce da fronteira — cada item do In-scope
    vira **um** FR, o comportamento que o ator observa como resultado; validações, ramos de
    erro, estados intermediários e gravações desse item são **ACs** dele na §7, nunca FRs
    irmãos. Item grande demais para um FR é item de In-scope mal fatiado: divida o **item**,
    não o FR. A cobertura não muda — muda a unidade.
...
<!-- Um FR por item do In-scope da §4.1 (princípio 11): passos, validações e ramos de
erro do mesmo item são ACs da §7, nunca FRs irmãos. -->

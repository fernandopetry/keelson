...
2. **Estratégia de branch (decisões 4.126/4.190)**: junto da tabela, proponha a estratégia — o default proposto vem de `git.branchStrategy` da ficha (campo ausente → `unica`): **`unica`** (todas as fatias empilhadas na branch do épico, sync com a main a cada fronteira de fatia, um PR ao final) · `por-fatia` (cada fatia na sua branch, regra 4.119 pura; o `/keelson:continue` passa a exigir merge da fatia da qual a próxima **depende** — 4.190). O Diretor pode sobrescrever o default épico a épico; a escolha entra na mesma confirmação abaixo e é **gravada no BRIEF**, que segue a única fonte de leitura. Nome da branch: dono em `sdd-conventions.md`, "Commit por marco" (com `git.branchNaming: "tracker-key"`, a key do Epic da Etapa 3 entra no nome — 4.192).
...
0.5. **Raiz do tracker (`jira.enabled` — decisão 4.191, mecânica no §16 do protocolo)**: crie o **Epic** do épico (stub honesto — summary do pedido, descrição mínima) e grave a key na linha `**Jira**:` do cabeçalho abaixo. Conector caído → pergunte ao Diretor se quer informar uma key manual; recusou → siga sem key, best-effort (§0). As Stories das fatias **não** nascem aqui — cada fatia cria a sua na própria largada, pendurada neste Epic.
1. Gravar `{docsRoot}/<slug-âncora>/briefs/BRIEF-<yyyy-mm-dd>-<descricao-curta>-epic.md` (contrato do BRIEF no `${CLAUDE_PLUGIN_ROOT}/docs/_meta/conventions/index-contract.md`, variação épico): cabeçalho com `**Branch**:` + `**Estratégia**:` (4.126) — e `**Jira**:` quando o item 0.5 criou/recebeu a key — e `**Origem**:`, pedido épico **verbatim**, perguntas respondidas e a decomposição confirmada como **fila viva** (4.125) — tabela com coluna `Estado`: fatias nascem `pendente`, exceto fatia cuja Q-ID `[bloqueia-núcleo]` pende de produto → nasce `aguardando-produto (Q-NN)`. `Status: Emitido`; o épico **não pareia com SPEC**. Entrada era BRIEF forjado (4.128) → atualize-o para `Status: decomposto` + caminho do épico (trilha nos dois sentidos; o NNN reservado dele não gera SPEC).

...
### Decisões herdadas
- **DEC-001-003** [herdada] Autorização do reenvio vive na camada de regra: `ConfirmacaoService::reenviar` recusa quem não é o dono (`pedidos.dono_id`) com `OperacaoNaoPermitida`; o controller só traduz em 403 — fonte: INDEX DEC-005-002.
- **DEC-001-004** [herdada] Só o `ParceiroFreteClient` fala com o parceiro; o service nunca usa `HttpGateway` nem HTTP direto — fonte: ficha (padrões de integração).

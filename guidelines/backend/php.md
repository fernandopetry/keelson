...
leitura de snapshot em `REPEATABLE READ`, e a corrida passa. E consulta travante que varre
**faixa ou prefixo** (condição que não fixa a chave única inteira — inclusive o prefixo de
um `UNIQUE` composto) segura next-key/gap lock **compartilhado** até o COMMIT: duas
transações seguradas no mesmo gap, cada uma pedindo *insert intention*, fecham deadlock
entre agregados vizinhos. Igualdade que fixa a chave única inteira trava só o registro,
sem gap.

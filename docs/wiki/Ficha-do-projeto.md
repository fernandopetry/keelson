...
| `review` | `true`/`false` — governa a **cutucada de encerramento** do hook `review-guard`: mudança de código **fora do ciclo** acima do limiar bloqueia o fim da sessão uma vez, pedindo revisão — e cala quando o veredito do `code-reviewer` já está no ledger da sessão e o diff de código é o mesmo que ele revisou (o comando marca a identidade do diff ao despachar o revisor e o registro do veredito a guarda; conteúdo mudado depois, mesmo com o mesmo tamanho, traz o lembrete de volta). **Não desliga** a revisão do ciclo — os gates 1–7 rodam sempre no `/keelson:implement` |

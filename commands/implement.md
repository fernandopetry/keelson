...
- (modo SUBAGENTS) tasks paralelas na **mesma working tree sem isolamento de árvore por task** (decisão 4.334 — 3ª camada da família 4.134/4.290, agora no executor): a árvore única é **recurso compartilhado de medição e edição**, não só um conjunto de arquivos — mesmo com Inclui disjuntos, duas verificações concorrentes na mesma árvore se atropelam. Caso real de campo: arquivos diferentes, zero conflito de conteúdo — e a contagem da suíte oscilou entre rodadas, uma edição desapareceu no meio da rodada e voltou a mão, e o developer improvisou `git worktree` para produzir evidência confiável. A régua de despacho pergunta por **recurso**, não só por aresta de dependência: paralelo exige árvore própria por task (worktree — a mesma disciplina que a 4.134 já impõe a gate que muta); sem ela, a wave roda **sequencial**, declarado no plano de execução (Etapa 2). O desenho completo do fluxo por worktree (branch por task, merge da wave) é leva própria — até lá, a resposta barata é serializar.

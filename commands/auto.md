...
4. **Push**: `git push` da branch para o remoto (`-u` na primeira vez). Após o push, **remova** `thoughts/local/run-state-<slug>.md` (guarda anti-parada — o run está entregue). **Sem abrir PR** (o dev revisa a branch e decide o merge). Se `jira.enabled`, aplicar o **protocolo de sync Jira** (`${CLAUDE_PLUGIN_ROOT}/skills/_shared/jira-sync-protocol.md`) em dois passos: **(a) reconciliação de fecho (§12)** sobre o slug — idempotente (§4), no-op barato quando os ganchos de `specify`/`tasks`/`implement` funcionaram, conserto do ciclo quando algum não rodou; "o gancho anterior deve ter criado" **não substitui** a passada — é exatamente o pressuposto que a reconciliação existe para verificar; **(b) comentar a branch/push na issue principal (§11)**. Leitura: §0–§1 + §11 + §12. Não leia o protocolo inteiro: localize os §§ com `grep -nE "^#+ §"` e leia §0 + §1 + os §§ citados aqui + os que eles referenciarem. Best-effort (§0). Guarde o estado medido pela reconciliação (Epic · Story · sub-tasks K/N · transições) — ele alimenta a linha do tracker no item 6.1.
